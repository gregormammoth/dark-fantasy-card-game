    ...
    <div className="overflow-x-clip bg-[#0b0908] text-[#e8ddcf]">
        ...
        <div className="grid grid-cols-2 overflow-hidden rounded-2xl border border-[rgba(201,162,74,.22)] bg-[rgba(20,16,15,.82)] shadow-[0_40px_90px_-30px_#000] backdrop-blur-[10px] md:grid-cols-4">
          {homeStats.map((stat, index) => (
            ...
            >
              ...
              <div className="mt-1.5 text-[11px] tracking-[0.14em] text-[#8a7f72]">
                {t(`site.stats.${stat.key}` as MessageKey)}
              </div>
            </div>
          ))}
        </div>
      ...
      >
        <div className="flex flex-col gap-[18px]">
          <p className="text-[11px] tracking-[0.28em] text-[#8a7f72]">{t('site.loreSection.eyebrow')}</p>
          <h2 className="font-cinzel text-4xl leading-snug text-[#f0dfcb]">
            {t('site.loreSection.titleLine1')}
            <br />
            {t('site.loreSection.titleLine2')}
          </h2>
          <p className="text-[15px] leading-relaxed text-[#a99c8d]">{t('site.loreSection.body')}</p>
          <div>
            <Link
      ...
      <RevealOnScroll>
        <section id="classes" className="mx-auto mt-[150px] max-w-[1180px] px-6 lg:px-10">
          <div className="mb-[34px] flex flex-col gap-4 sm:flex-row sm:items-baseline sm:justify-between">
            <div>
              <p className="mb-2.5 text-[11px] tracking-[0.28em] text-[#8a7f72]">
                {t('site.classesSection.eyebrow')}
              </p>
              <h2 className="font-cinzel text-[34px] text-[#f0dfcb]">{t('site.classesSection.title')}</h2>
            </div>
            <Link
              href="/classes"
              className="border-b border-[rgba(201,162,74,.3)] pb-0.5 text-[13px] tracking-[0.06em] text-[#c7bba9] transition hover:text-[#f0c979]"
            >
              {t('site.classesSection.link')}
            </Link>
          </div>
      ...
      <RevealOnScroll>
        ...
        >
          ...
          <div className="flex flex-col gap-[18px]">
            <p className="text-[11px] tracking-[0.28em] text-[#8a7f72]">{t('site.worldSection.eyebrow')}</p>
            <h2 className="font-cinzel text-4xl leading-snug text-[#f0dfcb]">{t('site.worldSection.title')}</h2>
            <p className="text-[15px] leading-relaxed text-[#a99c8d]">{t('site.worldSection.body')}</p>
      ...
      <RevealOnScroll className="mx-auto mt-[150px] max-w-[1180px] px-6 lg:px-10">
        <div className="grid gap-[22px] md:grid-cols-3">
          {homeFeatureKeys.map((key) => (
            ...
            >
              <p className="text-[10px] tracking-[0.2em] text-ember-400">
                {t(`site.features.${key}.tag` as MessageKey)}
              </p>
              <h3 className="mt-2.5 font-cinzel text-lg text-[#f0dfcb]">
                {t(`site.features.${key}.title` as MessageKey)}
              </h3>
              <p className="mt-2.5 text-[13px] leading-relaxed text-[#a99c8d]">
                {t(`site.features.${key}.desc` as MessageKey)}
              </p>
            </div>
          ))}
        </div>
      ...
      <section className="mx-auto mt-[150px] max-w-[1180px] px-6 lg:px-10">
        <div className="relative overflow-hidden rounded-[20px] border border-[rgba(201,162,74,.3)] bg-[radial-gradient(900px_300px_at_50%_-30%,rgba(201,162,74,.16),transparent),linear-gradient(180deg,#171210,#0d0a09)] px-8 py-[70px] text-center sm:px-[60px]">
          ...
          <p className="relative text-[11px] tracking-[0.3em] text-[#8a7f72]">{t('site.cta.eyebrow')}</p>
          <h2 className="relative mt-3.5 font-cinzel text-[42px] text-[#f3e6d6]">{t('site.cta.title')}</h2>
          <p className="relative mx-auto mt-3.5 max-w-[480px] text-[15px] text-[#a99c8d]">{t('site.cta.body')}</p>
          <Link
            href="/play"
